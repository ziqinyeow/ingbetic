...
import { bertNormalizer, bertPreTokenizer, wordPieceDecoder } from "tokenizers";

export class AutoTokenizer {
  static async from_pretrained(tokenizerPath) {
    const response = await fetch(tokenizerPath);
    const json = await response.json();
    const tokenizer = Tokenizer.fromConfig(json);
    return tokenizer;
  }
}
    ...
    switch (config.type) {
      ...
      case "BertNormalizer":
        return bertNormalizer({
          cleanText: config.normalizer.clean_text,
          handleChineseChars: config.normalizer.handle_chinese_chars,
          lowercase: config.normalizer.lowercase,
          stripAccents: config.normalizer.strip_accents,
        });
      case "WordPiece":
        return wordPieceDecoder(
          (prefix = config.decoder.prefix),
          (cleanup = config.decoder.cleanup)
        );
      case "Metaspace":
